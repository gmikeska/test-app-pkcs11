...
use bdk_wallet::bitcoin::{
    self, absolute::LockTime, transaction::Version, Address, Amount, FeeRate, Network, OutPoint,
...
fn bitcoin_batched_migration_chains_fee_change_old_fed_hsm() {
    ...
    let Some(env) = dev_env() else {
        eprintln!("skipping bitcoin_batched_migration_chains_fee_change_old_fed_hsm: dev HSM unavailable");
        return;
    };
